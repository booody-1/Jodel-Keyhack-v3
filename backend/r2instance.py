...
import r2pipe
import re
from decrypt import CLIENT_SECRET_SIZE
...
class R2Instance:
    ...
    def extract_bytes(self):
        ...
        instructions = [d for d in self.r2.cmd(R2_DISASSEMBLE_INSTRUCTIONS.format(
            self.function_name)).split('\r') if 'mov' in d and 'eax' in d]
        ...
        return [int(''.join(OrderedDict(sorted(instr.items())).values())[x:x + 2], 16) for x in range(0, CLIENT_SECRET_SIZE*2, 2)]
